...
1. El primer paso es crear la carpeta del país y la división geográfica de la cual se quiere cargar la capa. Para ello se puede correr la función `new_geo_pipeline()` quien, previo a crear la carpeta, va a revisar si la misma ya existe (tanto a nivel país como subregión).

```r
new_geo_pipeline(country = "Argentina", 
                    geo_divisiont = "Department")
```


1. El segundo paso es cargar la capa geográfica. El formato para trabajar la geometría debe ser **topoJSON** (a diferencia del GeoJSON, este formato es mucho más compacto) por lo que es necesario o bien cargar de forma manual la capa geográfica en esta extensión dentro de la carpeta creada o realizar el procesamiento necesario para ello. En caso de esto último, lo óptimo es crear un **script.R** con el paso a paso para garantizar la reproducibilidad del proceso. La función `new_geo_pipeline()` asiste en ello creando una versión vacía del script.
